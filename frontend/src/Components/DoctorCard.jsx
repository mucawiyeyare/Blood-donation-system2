...
// The photo sits flush against the card's top edge (no inset padding), with padding starting below it.
...
function DoctorCard({ doctor, preview = false, adminActions = null, dimmed = false, titled = true }) {
  ...
  return (
    <article
      className={`relative flex h-full flex-col overflow-hidden rounded-3xl border border-line/60 bg-white shadow-[0_12px_40px_-18px_rgba(15,60,140,0.3)] transition-shadow duration-300 hover:shadow-[0_18px_50px_-18px_rgba(15,60,140,0.4)] ${
        dimmed ? "opacity-50" : ""
      }`}
    >
      {adminActions && (
      ...
      <button
        ...
        className={`block w-full overflow-hidden bg-soft ${preview ? "cursor-default" : ""}`}
      >
        {doctor.photo ? (
